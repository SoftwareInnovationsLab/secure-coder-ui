...
function Exercise() {
  ...
  const handleSubmitCode = async () => {
    try {
      ...
      console.log(JSON.stringify({ exerciseId, code, language, input,result, exercise }));
    } catch (error) {
  ...
  const handleGetHints = async () => {
    const payload = {
      ...
      code: exercise.sourceCode || "",          // whatever field your YAML exposes
      cwe: exercise.category                    // optional metadata
    };
  
    try {
      const res = await fetch(
        `${process.env.REACT_APP_AI_TUTOR_URL}/api/hints`,
        {
          method: "POST",
